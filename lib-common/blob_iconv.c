...
static iconv_t get_iconv_handle(const char *to, const char *from)
{
    ...
    current_iconv = current_iconv >= ICONV_HANDLES_CACHE_MAX ? 0 : current_iconv;
    ...
    for (i = 0; i < ICONV_HANDLES_CACHE_MAX; i++) {
        if (iconv_handles[i].to && iconv_handles[i].from
            ...
            if (iconv_close(iconv_handles[i].handle) < 0) {
                e_debug(1, "An handle of iconv cannot be closed");
            } else {
                res++;
            }
        }
                           ...
                           const char *type_hint)
{
    int i;
    int enc = ENC_ASCII;
    const char *s = blob_get_cstr(src) + offset;
    
    if (!src) {
        return -1;
    }

    /* FIXME: The end condition is buggy. */
    for (i = 0; i < src->len && s[i]; i++) {
        if (s[i] > 0x7F) {
            if (enc == ENC_ASCII) {
        ...
        if (s[i] >= 0x80 && s[i] <= 0x9F) {
            ...
            if (s[i] == 0x83 || s[i] == 0x88 || s[i] == 0x89 || s[i] == 0x9c) {
                ...
                e_debug(1, "Win-1252 tres probable : %x\n", s[i]);
                if (s[i] != 0x9c) {
        ...
        if (s[i] >= 0xA0 && s[i] <= 0xBF) {
            ...
            e_debug(1, "Win-1250 quite probable, ISO-8859-1 probable : %x\n", s[i]);
            enc = (enc != ENC_ISO_8859_1) ? ENC_WINDOWS_1250 : enc;
            continue;
        }
        
        if (s[i] >= 0xC0 && s[i] <= 0xDF) {
            /* FIXME: check i < src->len */
            if (s[i + 1] == 0 || s[i + 1] < 0x80) {
                /* UTF 8 Impossible here */
                e_debug(1, "Win-1250, ISO-8859-1 equaly probable : %x %x\n",
                        s[i], s[i + 1]);
                if (enc == ENC_UTF8) {
                    enc = ENC_ISO_8859_1;
                }
            ...
            if (s[i + 1] >= 0x80 && s[i + 1] <= 0xBF) {
                enc = ENC_UTF8;
                e_debug(1, "UTF-8 very probable : %x %x\n", s[i], s[i + 1]);
                break;
            }
        }
        ...
        if (s[i] >= 0xE0 && s[i] <= 0xEF) {
            ...
            if ((s[i + 1] >= 0x80 && s[i + 1] <= 0xBF) &&
                (s[i + 2] >= 0x80 && s[i + 2] <= 0xBF)) {
                    enc = ENC_UTF8;
                    e_debug(1, "UTF-8 extremly probable : %x %x\n", s[i],
                            s[i + 1]);
                    break;
            } else {
                if (s[i] == 0xE0 || enc == ENC_UTF8) {
                    enc = ENC_ISO_8859_1;
                    e_debug(1, "ISO-8859-1 a little probable \n");
                    continue;
                }
            }
        }
                     ...
                     int *chosen_encoding)
{
    ...
    blob_resize(dst, 0);
    /* FIXME: Should use r_known_encodings ? */
    for (i = 0; known_encodings[i]; i++) {
        if (strcasecmp(type_hint, known_encodings[i]) == 0) {
            found = true;
            break;
        }
    }
    if (!found) {
        e_debug(1, "Skip type :%s\n", type_hint);
                         ...
                         int *chosen_encoding)
{
    ...
    if (blob_append_file_data(&tmp, filename) < 0) {
        e_debug(1, "Unable to append data from '%s'\n", filename);
        blob_wipe(&tmp);
        return -1;
    }
