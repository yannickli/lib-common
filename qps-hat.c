...
/* memory scan helpers (SSE optimized) {{{ */
...
#else
    for (; pos + 4 <= len; pos += 4) {
        if (u16[pos + 0]) return pos + 0;
        if (u16[pos + 1]) return pos + 1;
        if (u16[pos + 2]) return pos + 2;
        if (u16[pos + 3]) return pos + 3;
    }
#endif
...
static ssize_t scan_non_zero32(const uint32_t u32[], size_t pos, size_t len)
{
#if defined(__SSE2__) || defined(__SSE4_1__)
    if (len - pos >= 4) {
#ifdef __SSE4_1__
...
static ALWAYS_INLINE bool is_128bits_zero(const void *v)
{
    ...
    return _mm_testz_si128(*(const __m128i*)v, *(const __m128i*)v);
#elif defined(__SSE2__)
    __m128i t = _mm_cmpeq_epi32(*(const __m128i *)v, (__m128i){ 0, });
    return _mm_movemask_epi8(t) == 0xffff;
#else
    const qhat_128_t *_v = v;
    return _v->l == 0 && _v->h == 0;
#endif
}

static bool is_memory_zero(const void *_data, uint32_t n)
{
...
static uint32_t count_non_zero8(const uint8_t *data, uint32_t n)
{
#if defined(__SSE2__)
    ...
    for (uint32_t i = 0; i < n; ) {
        ...
        for (uint32_t j = 0; j < 63 && i < n; j++, i += 64) {
            acc0 -= __builtin_ia32_pcmpeqb128(*(__v16qi *)(data + i +  0), zero);
            acc1 -= __builtin_ia32_pcmpeqb128(*(__v16qi *)(data + i + 16), zero);
            acc2 -= __builtin_ia32_pcmpeqb128(*(__v16qi *)(data + i + 32), zero);
            acc3 -= __builtin_ia32_pcmpeqb128(*(__v16qi *)(data + i + 48), zero);
        }
...
static uint32_t count_non_zero16(const uint16_t *data, uint32_t n)
{
#if defined(__SSE2__)
    const __v8hi zero = { 0, };
    __v8hi acc0 = zero;
    ...
    for (uint32_t i = 0; i < n; i += 32) {
        acc0 -= __builtin_ia32_pcmpeqw128(*(__v8hi *)(data + i +  0), zero);
        acc1 -= __builtin_ia32_pcmpeqw128(*(__v8hi *)(data + i +  8), zero);
        acc2 -= __builtin_ia32_pcmpeqw128(*(__v8hi *)(data + i + 16), zero);
        acc3 -= __builtin_ia32_pcmpeqw128(*(__v8hi *)(data + i + 24), zero);
    }
...
static uint32_t count_non_zero32(const uint32_t *data, uint32_t n)
{
#if defined(__SSE2__)
    const __v4si zero = { 0, };
    __v4si acc0 = zero;
    ...
    for (uint32_t i = 0; i < n; i += 16) {
        acc0 -= __builtin_ia32_pcmpeqd128(*(__v4si *)(data + i +  0), zero);
        acc1 -= __builtin_ia32_pcmpeqd128(*(__v4si *)(data + i +  4), zero);
        acc2 -= __builtin_ia32_pcmpeqd128(*(__v4si *)(data + i +  8), zero);
        acc3 -= __builtin_ia32_pcmpeqd128(*(__v4si *)(data + i + 12), zero);
    }
...
static uint32_t count_non_zero64(const uint64_t *data, uint32_t n)
{
#if defined(__SSE4_1__)
    const __v2di zero = { 0, };
    __v2di acc0 = zero;
    ...
    for (uint32_t i = 0; i < n; i += 8) {
        acc0 -= __builtin_ia32_pcmpeqq(*(__v2di *)(data + i + 0), zero);
        acc1 -= __builtin_ia32_pcmpeqq(*(__v2di *)(data + i + 2), zero);
        acc2 -= __builtin_ia32_pcmpeqq(*(__v2di *)(data + i + 4), zero);
        acc3 -= __builtin_ia32_pcmpeqq(*(__v2di *)(data + i + 6), zero);
    }
...
static uint32_t count_non_zero128(const qhat_128_t *data, uint32_t n)
{
    register uint32_t acc0 = 0, acc1 = 0, acc2 = 0, acc3 = 0;
...
Z_GROUP_EXPORT(qhat_sse)
{
    srand(0);

#define DO_TEST(Size, Count, Get)                                            \
    __attribute__((aligned(4096)))                                           \
    qhat_##Size##_t v[Count];                                                \
    int set  = 0;                                                            \
                                                                             \
    p_clear(&v, 1);                                                          \
