...
`WWW_DEV` (boolean)::
    If set, the web bundles are build so as to help active development, for
    example:
      * source maps are improved
      * watchers are activated, so that a change is immediately taken into
        account in the bundles.
    Bear in mind that, as watchers are activated, the webpack tasks and
    thus the `waf` command *does not end*.
