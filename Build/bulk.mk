...
__setup_buildsys_tags: | __setup_buildsys_trampoline
	...
	cd $/ && $(var/toolsdir)/ctags.sh "$(TAGSOPTION)" "$(TAGSOUTPUT)"
...
ale: | __setup_buildsys_trampoline
	...
	echo "let g:ale_c_cc_options = '" >> $/.local_vimrc.vim
	echo ' $(CLANGFLAGS) $(libxml2_CFLAGS) $(openssl_CFLAGS) $(jni_CFLAGS) $(python2_CFLAGS)' | sed -e 's/^/    \\ /g' >> $/.local_vimrc.vim
	echo "\\'" >> $/.local_vimrc.vim
...
__dump_targets:
	...
	$(foreach v,$(filter %LINKER %LIBS %COMPILER %FLAGS %CFLAGSBASE %INCPATH %JSONPATH %TSPATH %CLASSRANGE %_SOVERSION %_NOCHECK %_CLASSPATH %_DIRECTORY %_NOGENERATED,$(filter-out MAKE%,$(.VARIABLES))),\
