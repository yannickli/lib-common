...
static int z_reply_gzip_empty(el_t el, int fd, short mask, el_data_t data)
{
    SB_1k(buf);
...
static int z_reply_no_content(el_t el, int fd, short mask, el_data_t data)
{
    SB_1k(buf);
...
{
    int (* query_cb)(el_t, int, short, el_data_t) = data.ptr;
...
Z_GROUP_EXPORT(httpc) {
    Z_TEST(unexpected_100_continue, "test behavior when receiving 100") {
        ...
        Z_ASSERT_LSTREQUAL(LSTR_SB_V(&body_g), LSTR_IMMED_V("Coucou"));
    ...
    Z_TEST(gzip_with_zero_length, "test Content-Encoding: gzip with Content-Length: 0") {
        ...
        Z_ASSERT_LSTREQUAL(LSTR_SB_V(&body_g), LSTR_IMMED_V(""));
