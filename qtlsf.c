...
static ALWAYS_INLINE void blk_set_prev(blk_hdr_t *blk, blk_hdr_t *prev)
{
    ...
    mem_tool_allow_memory(ptr, sizeof(*ptr), false);
    *ptr = prev;
}
...
static inline void blk_insert(tlsf_pool_t *mp, blk_hdr_t *blk, size_t bsz)
{
    ...
    mem_tool_allow_memory(blk, offsetof(blk_hdr_t, prev_hdr), false);
...
static ALWAYS_INLINE void arena_insert(tlsf_pool_t *mp, arena_t *a)
{
    ...
    mem_tool_malloclike(blk_next(blk, blk_size(blk))->data, 0, 0, 0);
...
ssize_t NEVER_INLINE tlsf_pool_add_arena(mem_pool_t *_mp, void *ptr, size_t size)
{
    ...
    mem_tool_malloclike(blk0->data, 0, 0, false);
...
void *tlsf_malloc(mem_pool_t *_mp, size_t size, mem_flags_t flags)
{
    ...
    mem_tool_disallow_memory(blk->data, size);
    mem_tool_malloclike(blk->data, asked, 0, false);
    if (!(flags & MEM_RAW))
                   ...
                   size_t oldsize, size_t newsize, mem_flags_t flags)
{
    ...
    if (newsize <= tsize) {
        ...
        blk->asked = asked;
        res = blk->data;
        mem_tool_freelike(ptr, oldsize, 0);
        mem_tool_malloclike(ptr, asked, 0, false);
        mem_tool_allow_memory(ptr, asked, true);
    } else
